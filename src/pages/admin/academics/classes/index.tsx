...
const Classes = () => {
  ...
  const processClassData = (classesData?: any[]) => {
    ...
    const mappedData = dataToProcess.map((item: any) => {
      ...
      const classLevelName =
        classLevelMap[classLevelValue] || `Level ${item.classLevel}`;
  ...
  const columns: Column[] = [
    ...
    },
    {
      ...
      format: (value: string, row: ClassModel) => (
        <span>{classLevelMap[row.rawClassLevel.toString()] || value}</span>
      ),
    },
    {
      id: "formTeacher",
  ...
  const actionColumn = {
    ...
    render: (row: ClassModel) => {
      ...
      const title = hasTeacher
        ? "Reassign Form Teacher"
        : "Assign Form Teacher";
