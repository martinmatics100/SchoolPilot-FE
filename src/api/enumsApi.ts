...
export const fetchSchoolLevels = async (selectedAccount: string | null) => {
    if (!selectedAccount) {
        throw new Error('No account selected');
    }

    const api = createApiClient({ selectedAccount });
    const response = await api.get(`${BASE_URL}/levels`);
    return response || [];
};

export const fetchSchoolTerms = async (selectedAccount: string | null) => {
    if (!selectedAccount) {
        throw new Error('No account selected');
    }

    const api = createApiClient({ selectedAccount });
    const response = await api.get(`${BASE_URL}/terms`);
    return response || [];
};

export const deleteSchoolTerm = async (selectedAccount: string | null, termValue: number) => {
    if (!selectedAccount) {
        throw new Error('No account selected');
    }
