...
export const ReusableTable: React.FC<ReusableTableProps> = ({
    ...
    const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (event.target.checked) {
            ...
            newSelection = selectedRows.filter((row) => !pageData.some((p) => p.id === row.id));
        }
        ...
        <Paper sx={{ width: '100%', overflow: 'hidden', bgcolor: theme.palette.background.default }}>
            ...
            <TableContainer sx={{ maxHeight: 300 }}>
                <Table stickyHeader aria-label="sticky table">
                    <TableHead>
                        <TableRow>
                            ...
                            {allColumns.map((column) => {
                                const isSortable = showSorting && columns.find((col) => col.id === column.id)?.sortable;
                    ...
                    <TableBody>
                        {loading ? (
                            <TableRow>
                                <TableCell colSpan={allColumns.length + (showCheckboxes ? 1 : 0)} align="center">
                                    Loading...
                                </TableCell>
                            </TableRow>
                        ) : pageData.length === 0 ? (
                            <TableRow>
                                <TableCell colSpan={allColumns.length + (showCheckboxes ? 1 : 0)} align="center" style={{ fontSize: "30px" }}>
                                    No data available
                                </TableCell>
                            </TableRow>
                        ) : (
            ...
            {showPagination && (
                <TablePagination
                    ...
                    onRowsPerPageChange={showRowsPerPage ? handleChangeRowsPerPage : undefined}
