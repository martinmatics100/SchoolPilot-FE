...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
    ...
    useEffect(() => {
        if (selectedAccount) {
            const storedBranches = localStorage.getItem(STORAGE_KEYS.SELECTED_BRANCHES);
            if (storedBranches) {
                try {
    ...
    useEffect(() => {
        const handleStorageChange = (e: StorageEvent) => {
            if (e.key === STORAGE_KEYS.SELECTED_ROLE) {
                const newRole = localStorage.getItem(STORAGE_KEYS.SELECTED_ROLE) as UserRoles | null;
                if (newRole !== role) {
                    setRole(newRole);
                    ...
                    if (authData) {
                        try {
                            ...
                            localStorage.setItem(STORAGE_KEYS.AUTH_DATA, encryptData({
                                ...parsedAuthData,
                                role: newRole
                            }));
                        } catch (error) {
        ...
        const checkRole = () => {
            const currentRole = localStorage.getItem(STORAGE_KEYS.SELECTED_ROLE) as UserRoles | null;
            if (currentRole !== role) {
                setRole(currentRole);
            }
        };
    ...
    useEffect(() => {
        if (accessToken) {
            ...
            localStorage.setItem(STORAGE_KEYS.AUTH_DATA, encryptData(authDataToPersist));
    ...
    const setAccountSelection = (accountId: string, branches: LocationModel[]) => {
        ...
        localStorage.setItem(STORAGE_KEYS.SELECTED_BRANCHES, JSON.stringify(branches));
    };
