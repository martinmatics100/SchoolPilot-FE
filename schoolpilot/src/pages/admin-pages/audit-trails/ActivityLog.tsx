...
// Create a styled span component for the "View Details" link
...
const ActivityLog = () => {
    ...
    const activityEntityTypeMap = useMemo(() => {
        return (
            enums?.ActivityEntityType?.reduce(
                ...
                { '1': 'Student', '2': 'Teacher' } // Fallback mapping
            ) || { '1': 'Student', '2': 'Teacher' }
        );
    }, [enums]);
    ...
    const fetchActivityLogs = async () => {
        ...
        try {
            ...
            const response = await api.get(
                `/v1/accounts/activity-logs?page=${page}&pageLength=${rowsPerPage}&sortBy=${sortBy}&order=${order}`
            );
    ...
    const processActivityLogData = () => {
        const mappedData = rawActivityLogs.map((item: any) => {
            ...
            return {
                ...
                category: item.category || 'Unknown', // Use API-provided category
        ...
        setData(mappedData);
        console.log('Processed Activity Logs:', mappedData); // Debug log
    };
    ...
    const fetchActivityLogDetails = async (id: string) => {
        try {
            const api = createApiClient({ selectedAccount });
            const response = await api.get(`/v1/accounts/activity-log/${id}/details`);
            console.log('Activity Log Details:', response);
            // Assuming the API returns an array of details directly
            // Adjust based on actual response structure, e.g., response.data if needed
            return response || [];
        } catch (error) {
    ...
    return (
        <div>
            ...
            <ReusableTable
                ...
                page={page - 1} // Convert to 0-based indexing for table
